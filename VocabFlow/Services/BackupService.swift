...
enum JSONImportSupport {
    static func detectKind(_ data: Data) -> VocabFlowJSONKind {
        guard !data.isEmpty,
    ...
    static func validate(_ data: Data, expecting kind: VocabFlowJSONKind) throws {
    ...
    static func message(for error: Error, expecting kind: VocabFlowJSONKind) -> String {
    ...
    private static func wrongFormatHint(expecting kind: VocabFlowJSONKind) -> String {
        switch kind {
        case .deckPack:
...
enum JSONImportError: LocalizedError {
    case emptyFile
    case invalidJSON
    case wrongFormat(VocabFlowJSONKind)
