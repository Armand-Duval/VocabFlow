...
> 校准分 ~82/100。双主线：**SRS 算法** + **Demo 感消除**（文字驱动 → 视觉优先）。  
> 状态：`[ ]` 待做 · `[~]` 进行中 · `[x]` 已完成
...
| Settings | 居中浅灰 **Settings** / 设置 | **保留弱化** | 底栏第四 Tab |
...
| ID | 项 | 状态 | 说明 |
|----|----|------|------|
| P0-1 | SRS 首次毕业 + Good/Easy 梯度 | [x] | UI 三档：不会(Again 短间隔) / 良好 / 简单；Hard 并入不会 |
...
| P0-6 | FSRS-4.5 替换简化 SM-2 | [x] | 保留 interval/ease 镜像字段；旧卡首次评分时迁移 |
...
| P1-2 | Share Extension 实机回归 + 边界修复 | [x] | 空文本、20s 超时、图片 OCR 失败文案 |
| P1-3 | 首次复习手势引导 | [x] | `review.hasSeenSwipeCoach` 遮罩示意 |
| P1-4 | Create 顶部快捷卡片区 | [x] | 已回退：OCR 放回原文区 |
| P1-5 | 「待制作卡片」Banner 强化 | [x] | `PendingCardsBannerView` |
| P1-6 | Deck 状态可视化 | [x] | 词库行 Due 比例细条 + 待复计数 |
| P1-7 | 首次打开一次性引导 | [x] | `FirstLaunchGateView` 产品介绍 |
| P1-8 | 报错改 Toast | [x] | Create / Preview / Settings / DeckStore / Account；致命确认仍用 sheet |
...
| ID | 项 | 状态 | 说明 |
|----|----|------|------|
| P2-1 | 复习统计看板 | [x] | 首页今日新学/复习 + 摘录/温习活动 |
| P2-2 | 子词库 Subdeck | [ ] | 多层级 deck（短期不做） |
| P2-3 | AI 额度 / 会员页 | [ ] | App Store 3.1.1 |
| P2-4 | 启动隐私同意弹窗 | [x] | 与首次引导合并；同意后才请求通知 |
...
- **2026-08-12** 主闭环收口：FSRS-4.5、首次引导+隐私同意、复习手势教练、Toast 化非致命报错、Share 超时/OCR 失败、今日统计看板；文档对齐四 Tab
